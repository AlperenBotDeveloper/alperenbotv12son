...
if(!hesapismi) {
  ...
  db.set(`hesapdurumuk4h1n-${message.author.id}`, "aktif");
      ...
      if(client.k4h1n.rastgelepara == true) {
        ...
        message.channel.send(`Başlangıç parası rastgele olarak **${randomizer} ${client.k4h1n.parabirimi}** hesabınıza yatırıldı!`)
      } else {
        if(client.k4h1n.rastgelepara == false) {
          ...
          db.set(`hesaptarihiçdayreay-${message.author.id}`, ay)
          db.set(`hesaptarihiçdayregün-${message.author.id}`, gün)
          db.add(`bakiyecdare-${message.author.id}`, client.k4h1n.başlangıçparası)
          message.channel.send(`Başlangıç parası olarak **${client.k4h1n.başlangıçparası} ${client.k4h1n.parabirimi}** hesabınıza yatırıldı!`)
        }
      }
